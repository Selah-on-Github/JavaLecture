package chap12_interface.board;
	...
	//인터페이스는 자동으로 무조건 상수만 사용할 수 있다.
	//상수는 무조건 값을 지정해 줘야 한다.
	//변수만 선언하면 에러가 난다.
	...
	// 인터페이스에서 작성하는 메소드는 무조건 추상메소드이기 때문에 구현부인 중괄호 블럭({})을 가질 수 없다.
	
	// 인터페이스도 추상클래스와 마찬가지로 객체를 만들지 못해서 (인스턴스화 불가능) 무조건 자식클래스를 만들어야 한다.
...
//	void insertBoard() {     //에러. 중괄호 블럭을 넣으면 에러.
//		
//	}
	...
	void selectBoard(); //게시글 조회하는 것.
	
	
	

	// 4. 부모 인터페이스에 새로운 기능을 추가하려면 모든 자식 클래스를 수정해야하는 문제점이 발생해서
	// dafault 메소드가 등장했다.
//	void selectBoardList(); //이렇게 만들기 보단,
